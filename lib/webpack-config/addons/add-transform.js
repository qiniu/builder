...
const { isDefaultPolyfill, isRuntimePolyfill } = require('../../constants/polyfill')
 ...
 */
const makeBabelLoaderOptions = (options, targets, { addPolyfill, polyfillType }) => {
  options = options || {}
  ...
  const defaultBabelPresetEnv = [babelPresetEnvName, {
    ...
    ...(
      // default polyfill
      addPolyfill
      && isDefaultPolyfill(polyfillType)
  ...
  const defaultBabelPluginTransformRuntime = [babelPluginTransformRuntimeName, {
    // TODO: corejs3 支持实例属性，如：[].includes
    // 但可能存在 default polyfill 不再可用的风险，需要大量测试，所以暂时不升级
    // https://babeljs.io/docs/en/babel-plugin-transform-runtime#polyfill
    corejs: 2
  }]
  ...
  return update(options, {
    ...
    plugins: {
      $set: (
        ...
        !babelPluginTransformRuntime && addPolyfill && isRuntimePolyfill(polyfillType)
