  ...
  return update(options, {
    presets: {
      $set: (options.presets || []).map(
        preset => {
          if (preset === 'env') {
            return ['env', { 'modules': false, targets: { browsers: ['last 2 versions'] } }]
          }
          if (preset && preset[0] === 'env') {
            return ['env', utils.extend({}, preset[1], { 'modules': false, targets: { browsers: ['last 2 versions'] } })]
          }
          return preset
        }
      ).map(makeBabelPreset)
          ...
          $push: [
            makeRule(extension, context, {
              ...
              options: {
                ...
                // ES 的情况下启用 esModule 
                // TS 的情况下禁用 esModule
                // 因为 svg-sprite 对 ts 不友好，在 ts 中引入 svg 文件只能通过 require
                // 这个时候把 esModule 禁用可以拿到正确的内容
                // 而 ES 中一般会通过 import 引入 svg 文件
                esModule: false,
                // TODO:
