...
// 找到 env 这个 preset，添加 { "modules": false }
// 注意后续可能要修改这边逻辑，考虑会对 import / export 进行转换的不一定只有 env 这个 preset
const adaptBabelLoaderOptions = (options, targets) => {
  if (!options) {
    return options
  }
  return update(options, {
    presets: {
      $set: (options.presets || []).map(
        preset => {
          if (preset === 'env') {
            return ['env', { 'modules': false, targets }]
          }
          if (preset && preset[0] === 'env') {
            return ['env', utils.extend({}, preset[1], { 'modules': false, targets })]
          }
          return preset
        }
      ).map(makeBabelPreset)
...
const adaptLoader = targets => ({ loader, options }) => {
  loader = makeLoaderName(loader)
  options = loader === 'babel-loader' ? adaptBabelLoaderOptions(options, targets) : options
...
const makeRule = (extension, context, targets, ...loaderList) => {
  const rule = {
    test: makeExtensionPattern(extension),
    use: loaderList.map(adaptLoader(targets))
  }
  ...
  switch(transform.transformer) {
    ...
    case transforms.stylus: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, targets, ...loaders)]
        } }
      })
      break
    }
    ...
    case transforms.babel: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, targets, { loader: 'babel', options: transform.config })]
        } }
      })
      break
    }
    ...
    case transforms.jsx: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, targets, {
            loader: 'babel',
    ...
    case transforms.flow: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, targets, { loader: transform.transformer, options: transform.config })]
        } }
      })
      break
    }
    ...
    case transforms.file: {
      config = update(config, {
        module: { rules: {
          $push: [
            makeRule(extension, context, targets, {
              loader: 'file',
    ...
    case transforms.vue: {
      ...
      const postcssOptionsInVue = makePostcssOptions({ autoprefixOptions: targets })
