...
module.exports = (config, key, transform, post) => {
  ...
  switch(transform.transformer) {
    ...
    case transforms.stylus:
      ...
      const cssOptions = utils.extend({
        importLoaders: 1
      }, transform.config && transform.config.cssOptions)
      const loaders = [
        { loader: 'style-loader' },
        // 需要保证 stylus, less, scss, 本身没有 'postcssOptions', 'cssOptions' 的 option        
      ...
      if (transform.transformer !== 'css') {
        loaders.push({
          ...
          options: omit(transform.config, ['postcssOptions', 'cssOptions'])
        })
      }
