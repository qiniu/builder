...
function makePostcssOptions({ autoprefixOptions }) {
  const fixedOptions = {...autoprefixOptions}
  ...
  switch(transform.transformer) {
    ...
    case transforms.stylus: {
      ...
      const postcssOptions = makePostcssOptions({
        autoprefixOptions: { browsers: targets.browsers}
      })
    ...
    case transforms.babel: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, { loader: 'babel', options: makeBabelLoaderOptions(transform.config, targets) })]
        } }
      })
      break
    }
